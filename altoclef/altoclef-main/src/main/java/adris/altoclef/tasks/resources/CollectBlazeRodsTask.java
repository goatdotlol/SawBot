...
public class CollectBlazeRodsTask extends ResourceTask {
    ...
    private static boolean isHoveringAboveLavaOrTooHigh(AltoClef mod, Entity entity) {
        int MAX_HEIGHT = 23;
        for (BlockPos check = entity.getBlockPos(); entity.getBlockPos().getY()
                - check.getY() < MAX_HEIGHT; check = check.down()) {
    ...
    protected Task onResourceTick(AltoClef mod) {
        ...
        if (mod.getEntityTracker().entityFound(BlazeEntity.class)) {
            ...
            if (_foundBlazeSpawner != null && toKill.isPresent()) {
                ...
                double sqDistanceToPlayer = nearest.squaredDistanceTo(mod.getPlayer().getPos());// _foundBlazeSpawner.getX(),
                                                                                                // _foundBlazeSpawner.getY(),
                                                                                                // _foundBlazeSpawner.getZ());
                ...
                if (sqDistanceToPlayer > SPAWNER_BLAZE_RADIUS * SPAWNER_BLAZE_RADIUS) {
                    ...
                    BlockHitResult hit = mod.getWorld()
                            .raycast(new RaycastContext(mod.getPlayer().getCameraPosVec(1.0F),
                                    kill.getCameraPosVec(1.0F), RaycastContext.ShapeType.OUTLINE,
                                    RaycastContext.FluidHandling.NONE, mod.getPlayer()));
                    if (hit != null
                            && hit.getBlockPos().getSquaredDistance(mod.getPlayer().getPos()) < sqDistanceToPlayer) {
        ...
        if (toKill.isPresent() && toKill.get().isAlive()) {
            ...
            return new adris.altoclef.tasks.entity.KillBlazeSmartTask(toKill.get());
        }

        // If the blaze spawner somehow isn't valid
        if (_foundBlazeSpawner != null && mod.getChunkTracker().isChunkLoaded(_foundBlazeSpawner)
                && !isValidBlazeSpawner(mod, _foundBlazeSpawner)) {
        ...
        if (_foundBlazeSpawner != null) {
            if (!_foundBlazeSpawner.isWithinDistance(mod.getPlayer().getPos(), 4)) {
                ...
                Optional<BlockPos> nearestFire = mod.getBlockTracker().getNearestWithinRange(_foundBlazeSpawner, 5,
                        Blocks.FIRE);
